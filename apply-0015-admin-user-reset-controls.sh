...
export async function resetUserState(params: {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(await getRepairErrorMessage(response, `${params.mode} reset failed with HTTP ${response.status}`));
    }
  ...
  app.post("/api/admin/hard-reset-user", authenticate, async (req: any, res) => {
    ...
    if (!confirmReset || String(confirmationText || '').trim() !== 'HARD RESET') {
      return res.status(400).json({
        ...
        message: "Type HARD RESET to confirm the hard reset action."
      });
    }
    ...
    try {
      ...
      if (!userRef || !userData || !userId) {
        return res.status(400).json({
          ...
          message: "Provide targetUserId, targetUsername, or targetEmail."
        });
      }
      ...
      const resetFields: any = {
        ...
        softResetAt: FieldValue.delete(),
        softResetBy: FieldValue.delete(),
...
print("Admin reset controls patch applied.")
PY
